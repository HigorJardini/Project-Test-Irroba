...
Route::namespace('Web\Admin')->prefix('admin')->name('admin')->middleware('auth', 'logout.user')->group(function () {
    ...
    Route::namespace('Classes')->prefix('classes')->middleware('permission:read-classes')->name('.classes')->group(function () {
        ...
        Route::get('view/{class_id}',                  'ClassesController@view')->middleware('permission:update-classes')->name('.view');
        Route::put('update/{class_id}',                'ClassesController@update')->middleware('permission:update-classes')->name('.update');
        Route::delete('delete/{class_id}',             'ClassesController@delete')->middleware('permission:delete-classes')->name('.delete');

        Route::get('request/{class_id}',               'ClassesController@request')->middleware('permission:request-classes')->name('.request');
        Route::delete('request/cancel/{class_id}',     'ClassesController@requestCancel')->middleware('permission:request-cancel-classes')->name('.request.cancel');

        Route::get('students/{class_id}',              'ClassesController@studentsIndex')->middleware('permission:read-classes-students')->name('.index.students');
